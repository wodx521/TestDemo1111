...
import com.lr.sia.ui.moudle3.adapter.MyGroupAdapter;
...
public class MyGroupFragment extends BasicFragment {
    ...
    private void getMyGroupsAction() {
        ...
        mRequestPresenterImp.requestPostToMap(MethodUrl.CHAT_MY_GROUPS, map);
    }
                ...
                switch (tData.get("code") + "") {
                    case "1": //请求成功
                        if (UtilTools.empty(tData.get("data") + "")) {
                            ...
                            mGroupList = (List<Map<String, Object>>) dataMap.get("data");
                            if (UtilTools.empty(mGroupList)) {
                                pageViewEmpty.setVisibility(View.VISIBLE);
                            } else {
                                pageViewEmpty.setVisibility(View.GONE);
                                myGroupAdapter.setmFriendList(mGroupList);
                            }
                        }
                        break;
                    case "0": //请求失败
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        rvList.setAdapter(myGroupAdapter);
